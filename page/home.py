...
def get_state():
    if "cap" not in st.session_state:
        ...
        # st.session_state.cap = cv2.VideoCapture("../../test/mitsuke.mp4")
...
def home():
    ...
    while st.session_state.cap.isOpened():
        ...
        if time.time() - start_time >2:
            ...
            # Plotlyの図表オブジェクトの作成
            fig = go.Figure()
            for idx, dot_product in enumerate(dot_product_list):
                # index = dot_product_list_index % dot_product_list_size
                # dot_product = np.concatenate([dot_product[index:], dot_product[:index]], axis=0)
                fig.add_trace(go.Scatter(x=list(range(len(dot_product))), y=dot_product, mode='lines', name=member[idx]))
